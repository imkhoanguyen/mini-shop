...
export class ProductFormComponent implements OnInit {
  ...
  loadVariants(variants: Variant[]) {
    variants.forEach((variant) => {
      ...
      if(variant.imageUrls) {
        const imageUrlsArray = variant.imageUrls.map(url =>
           this.fb.group({
              url: url,
              isMain: false,
            })
        );
        variantGroup.setControl('imageUrls', this.fb.array(imageUrlsArray) as FormArray);
      }
  ...
  onFileSelected(event: Event, variantIndex: number) {
    ...
    const imageUrls = this.variants.at(variantIndex).get('imageUrls') as FormArray;
    if (input?.files) {
  ...
  selectMainImage(variantIndex: number, imageIndex: number) {
    const imageUrls = (this.variantsForm.get('variants') as FormArray).at(variantIndex).get('imageUrls') as FormArray;
  ...
  isMainImage(variantIndex: number, imageIndex: number) {
    ...
    .at(variantIndex).get('imageUrls')?.value[imageIndex].isMain;
  }
  ...
  addImages(variantId: number, variantIndex: number) {
    ...
    const hasMainImage = variantImages.controls.some((image: any) => image.get('isMain').value);
  ...
  addVariants(productId: number) {
    this.variants.controls.forEach((variantGroup, index) => {
      ...
      this.variantService.addVariant(variantAdd).subscribe(
        ...
        (error) =>
          this.showMessage('error','Lỗi khi thêm biến thể.')
      );
    });
  }
  updateVariant() {
    this.variants.controls.forEach((variantGroup, index) => {
      ...
      this.variantService.updateVariant(variantUpdate).subscribe(
        ...
        (error) =>
          this.showMessage('error','Lỗi khi cập nhật biến thể.')
      );
    });
  }
  // ----------------- ProductCU ----------------
    ...
    if (this.productForm.get('id')?.value) {
      ...
      this.productService.addProduct(productAdd).subscribe(
        (response: any) => {
          ...
          this.variantsForm.value.variants.forEach((variant: any, index: number) => {
